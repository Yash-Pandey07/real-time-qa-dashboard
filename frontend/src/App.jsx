...
export default function App() {
  ...
  useEffect(() => {
    fetch('/api/ci/runs?limit=100').then(r => r.json()).then(d => { if (d.runs) setCiRuns(d.runs); }).catch(() => {});
    fetch('/api/ci/heatmap').then(r => r.json()).then(d => { if (d.grid) setHeatmapData(d.grid); }).catch(() => {});
    fetch('/api/jira/issues?limit=50').then(r => r.json()).then(d => {
      if (d.issues) setJiraData(prev => ({ ...prev, allIssues: d.issues, metrics: d.metrics || {} }));
    }).catch(() => {});
    fetch('/api/jira/projects').then(r => r.json()).then(d => {
      if (d.projects) setJiraData(prev => ({ ...prev, projects: d.projects }));
    }).catch(() => {});
    fetch('/api/tests/cycles').then(r => r.json()).then(d => {
      if (d.testCycles) setTestData(prev => ({ ...prev, testCycles: d.testCycles, summary: d.summary || {} }));
    }).catch(() => {});
    fetch('/api/tests/executions?limit=100').then(r => r.json()).then(d => {
      if (d.testExecutions) setTestData(prev => ({ ...prev, testExecutions: d.testExecutions }));
    }).catch(() => {});
    fetch('/api/bottlenecks').then(r => r.json()).then(d => {
